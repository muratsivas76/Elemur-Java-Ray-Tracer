...
public class ElenaRayTracerGUI extends JFrame {
  ...
  public ElenaRayTracerGUI() {
    ...
    JButton calculateBtn = getJButton("Calculate");
  ...
  private JPanel createCameraTab() {
    ...
    JLabel xylbl = getJLabel("Double click on renderPanel for xy info.");
  ...
  private JPanel createShapesTab() {
    ...
    changeTransformsButton.addActionListener(new ActionListener() {
        public void actionPerformed(ActionEvent evt) {
		  ...
		  //for (EMShape s : Utilities.shapes) newScene.addShape(s);
		  //for (Light l : Utilities.lights) newScene.addLight(l);

		  //currentTracer = new ElenaMuratRayTracer(newScene, width, height, bgColor);
		  //currentTracer.setCamera(camera);
		  //if (shadowColor != null) currentTracer.setShadowColor(shadowColor);
  ...
  private final class RenderPanel extends JPanel implements MouseListener {
	...
	public void mouseClicked(MouseEvent e) {
		...
		JOptionPane.showMessageDialog(renderPanel, "<html><body><b><font color=\"blue\" size=\"7\">" + pstr + "</font></b></body></html>");
