...
public final class ColorUtil {
  ...
  public static Color applyShadowColor(Color original, Color shadowColor) {
    ...
    // result = shadowColor * alpha + original * (1 - alpha)
    // RGB  normalize
    float shadowR = shadowColor.getRed() / 255.0f;
    float shadowG = shadowColor.getGreen() / 255.0f;
    float shadowB = shadowColor.getBlue() / 255.0f;
    
    float origR = original.getRed() / 255.0f;
    float origG = original.getGreen() / 255.0f;
    float origB = original.getBlue() / 255.0f;
    
    // Alpha blending
    float resultR = shadowR * alpha + origR * (1 - alpha);
    float resultG = shadowG * alpha + origG * (1 - alpha);
    float resultB = shadowB * alpha + origB * (1 - alpha);
    
    // 0-255
    return new Color(
        clamp((int)(resultR * 255)),
        clamp((int)(resultG * 255)),
        clamp((int)(resultB * 255))
    );
  }
